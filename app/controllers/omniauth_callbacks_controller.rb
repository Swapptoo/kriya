    ...
    if params[:provider] == 'slack'
      ...
      if response['ok']
        ...
        if room_id.present?
          ...
          channel = channels.detect { |c| slack_channel.channel_id == c.id }
          if channel.nil?
          ...
          slack_msg = room.messages.find_or_create_by(seen: true, body: 'Do you use Slack?', user: room.manager, msg_type: "slack-#{user.type}")
